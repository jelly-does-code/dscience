...
try:
    import cudf
    cudf.DataFrame()  # Attempt to use cudf to confirm compatibility
    cudf_available = 1
    print("cuDF is available and compatible.")
except Exception:
    cudf_available = 0
    print("cuDF is not available or incompatible, using modin.pandas instead.")
    from modin.pandas import DataFrame, NA, concat, read_csv
    import ray
    ...
    if model_map[name]['refit'] == 1 or not isfile(f'../models/{name}_best.json'):
        ...
        if model_map[name]['handles_cat']:
            if 'CatBoost' in name:
                ...
                model_map[name]['model'] = model_map[name]['model'](**model_map[name]['params']).fit(data_map['X_train']._to_pandas(), data_map['y_train']._to_pandas(), data_map['cat_cols_engineered'])
            else:
...
def predict(name, data_map, model_map, predict_data):
    ...
    elif proba_func == 'predict_proba' and 'CatBoost' not in name:
        model_map[name]['pred_proba'] = model.predict_proba(X_to_predict)[:, 1]
    else:
        model_map[name]['pred_proba'] = model.predict_proba(X_to_predict._to_pandas())[:, 1]
    ...
    if runtime_map['calculate_kfold']:
        for name in model_map:
            if model_map[name]['refit'] == 1:
                ...
                model_map[name]['cross_val_score'] = cross_val_score(model_map[name]['model'], data_map['X_train']._to_pandas(), data_map['y_train']._to_pandas(), cv=runtime_map['kfold'], scoring=runtime_map['scoring'])
...
def main():
    ...
    for name, model_info in model_map.items():
        if model_info['refit'] == 1:
            ...
            submission_curr = DataFrame({data_map['index_cols'][0]: X_pred_data.index, data_map['target_cols'][0]: predictions_curr})
            submission_best = DataFrame({data_map['index_cols'][0]: X_pred_data.index, data_map['target_cols'][0]: predictions_best})
