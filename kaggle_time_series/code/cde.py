...
from functions import convert_sparse_to_df, update_maps_from_config, log, predict
...
def tune_train(data_map, model_map, runtime_map):
    ...
    print(name)
    print(f'Training: checking performance with chosen performance metric for {name} .. ({runtime_map["scoring"]})')
    model_map[name]['perf'] = mean_squared_log_error(data_map['y_test'], model_map[name]['pred_proba'])
...
def main():
    data_map, model_map, runtime_map = update_maps_from_config('config/data_map.json', 'config/model_map_regressor.json', 'config/runtime_map.json')
    ...
    for name, model_info in model_map.items():
        if model_info['refit'] == 1:
            ...
            predictions_curr = predict(model_info['model'], X_pred_data)
            predictions_best = predict(model_info['best_model'], X_pred_data)
